...
func TestDirectoryDiscoveryCapturesFilteredWafResponse(t *testing.T) {
	...
	if target.WafDetection == nil || target.WafDetection.Request == nil || target.WafDetection.Request.Response == nil {
		t.Fatal("expected target-level WAF detection with its request response")
	}
