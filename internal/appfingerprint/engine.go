...
import (
	...
	apiapplication "github.com/Method-Security/webscan/internal/appfingerprint/modules/apiapplication"
	"github.com/Method-Security/webscan/internal/appfingerprint/modules/cloudbucket"
)

type Module interface {
	ModuleRun(target string, config *webscan.AppFingerprintConfig) (*webscan.AppFingerprintAttempt, []string)
	AnalyzeResponse(response *webscan.AppFingerprintResponseInfo) bool
}
...
func (e *Engine) Run(ctx context.Context, target string) (*webscan.AppFingerprintAttempt, []string) {
...
func (e *Engine) Launch(ctx context.Context) (*webscan.AppFingerprintReport, error) {
	report := webscan.AppFingerprintReport{ResourseType: e.Config.ResourceType, Config: e.Config}
	errors := []string{}
	...
	var resources []*webscan.AppFingerprintResourceInfo
	for _, target := range e.Config.Targets {
		var attempts []*webscan.AppFingerprintAttempt
		...
		if e.Config.SuccessfulOnly {
			successfulAttempts := []*webscan.AppFingerprintAttempt{}
		...
		resource := webscan.AppFingerprintResourceInfo{Target: target, Attempts: attempts}
