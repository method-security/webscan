...
// IEEE OUI prefix, registered to a specific manufacturer. Templates
// emit method-oui-lookup-mac with the raw MAC string; this helper
// normalizes it and returns the vendor name (or empty if unknown).
//
...
// ouiLookupTable is the in-memory mirror of
// utils/nuclei/templates/discover/application/_data/oui.yaml. Per-
// cluster PRs (Cluster F Ubiquiti, Cluster I Raisecom, Cluster P
// broader sweep) populate it. Empty at SEC-702.A seed-time by
...
var ouiLookupTable = map[string]string{
...
func normalizeOuiPrefix(mac string) string {
	...
	for _, r := range mac {
		...
		case r >= 'a' && r <= 'f':
			b.WriteRune(r - 32)
		default:
			// Skip separators (: - . space) and any other character.
		}
	}
	hex := b.String()
	if len(hex) < 12 {
